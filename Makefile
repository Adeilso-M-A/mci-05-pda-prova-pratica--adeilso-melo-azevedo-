...
.PHONY: help status add commit push push-force pull fetch log branch checkout diff stash stash-pop rebase rebase-continue rebase-abort build up down migrate logs
...
up: ## Sobe os containers em segundo plano
	$(COMPOSE) up -d
